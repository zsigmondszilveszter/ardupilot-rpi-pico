...
void Storage::_mark_dirty(uint16_t loc, uint16_t length)
{
    ...
    for (uint16_t line = loc >> RP2040_STORAGE_LINE_SHIFT;
         line <= end >> RP2040_STORAGE_LINE_SHIFT;
         line++) {
        _dirty_mask |= (1U << line);
    }
}
...
void Storage::_timer_tick(void)
{
    ...
    uint16_t i;
    for (i = 0; i < RP2040_STORAGE_NUM_LINES; i++) {
        if (_dirty_mask & (1U << i)) {
            break;
        }
    }
    if (i >= RP2040_STORAGE_NUM_LINES) {
        return;
    }
    ...
    {
        WITH_SEMAPHORE(sem);
        memcpy(tmpline, &_buffer[RP2040_STORAGE_LINE_SIZE * i], RP2040_STORAGE_LINE_SIZE);
    }

    // write to file
    uint32_t offset = RP2040_STORAGE_LINE_SIZE * i;
    if (AP::FS().lseek(_storage_fd, offset, SEEK_SET) != (off_t)offset) {
        return;
    }
    if (AP::FS().write(_storage_fd, tmpline, RP2040_STORAGE_LINE_SIZE) != RP2040_STORAGE_LINE_SIZE) {
        return;
    }
    ...
    WITH_SEMAPHORE(sem);
    if (memcmp(tmpline, &_buffer[RP2040_STORAGE_LINE_SIZE * i], RP2040_STORAGE_LINE_SIZE) == 0) {
        _dirty_mask &= ~(1U << i);
    }
}
...
#endif // CONFIG_HAL_BOARD == HAL_BOARD_RP2040CHIBIOS
