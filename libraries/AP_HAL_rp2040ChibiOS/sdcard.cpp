...
bool sdcard_init()
{
    ...
    mmcconfig.spip  = static_cast<Rp2040ChibiOS::SPIDevice*>(device.get())->get_driver();
...
void spiSelectHook(SPIDriver *spip)
{
    ...
    static_cast<Rp2040ChibiOS::SPIDevice*>(device.get())->acquire_bus(true, true);
    hal.gpio->write(RP2040_SPI_CS_FOR_SDCARD, 0);
}
...
void spiUnselectHook(SPIDriver *spip)
{
    ...
    hal.gpio->write(RP2040_SPI_CS_FOR_SDCARD, 1);
    // acquire_bus(false, true): releases ChibiOS bus lock, clears cs_forced.
    static_cast<Rp2040ChibiOS::SPIDevice*>(device.get())->acquire_bus(false, true);
    if (!sdcard_sem_held) {
