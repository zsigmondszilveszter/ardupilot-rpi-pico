...
#define SCR_PRESCALER 0

static inline void cs_select(uint8_t pin) {
    asm volatile("nop \n nop \n nop");
    hal.gpio->write(pin, 0);  // Active low
    asm volatile("nop \n nop \n nop");
}
...
Rp2040ChibiOS::SPIDesc SPIDeviceManager::device_table[] = { HAL_SPI_DEVICE_LIST };
    ...
    , device_desc(_device_desc)
{
    ...
    hal.gpio->pinMode(device_desc.cs_line, 1);
    cs_deselect(device_desc.cs_line);
}
...
{
    return (RP_PERI_CLK / (SCR_PRESCALER + 1)) / _frequency;
}
...
bool SPIDevice::acquire_bus(bool set, bool skip_cs)
{
    ...
    if (!set && cs_forced) {
        if(!skip_cs) {
            cs_deselect(device_desc.cs_line);                              /* Slave Select de-assertion.       */
        }
        ...
        if(!skip_cs) {
            cs_select(device_desc.cs_line);                                    /* Slave Select assertion.          */
        }
        cs_forced = true;
    }
    return true;
