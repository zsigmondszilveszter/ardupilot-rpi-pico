...
struct SPIDesc {
    SPIDesc(const char *_name, uint8_t _bus,
            uint8_t _device, ioline_t _miso_line,
            ioline_t _mosi_line, ioline_t _sck_line, ioline_t _cs_line,
            uint32_t _mode, uint32_t _lowspeed, uint32_t _highspeed)
        : name(_name), bus(_bus), device(_device),
          miso_line(_miso_line), mosi_line(_mosi_line), 
          sck_line(_sck_line), cs_line(_cs_line), 
          mode(_mode),
