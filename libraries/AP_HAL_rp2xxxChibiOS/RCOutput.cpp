...
void RCOutput::init() {
    ...
    palSetLineMode(RP2xxx_RC_OUT0, PAL_MODE_ALTERNATE_PWM);
...
void RCOutput::set_output_mode(uint32_t mask, enum output_mode mode) {
    bool driver_affected[RP2xxx_NR_PWM_PERIPH_ENABLED] = {};
    for (uint8_t chan = 0; chan < NUM_CHANNELS; chan++) {
        if (!(mask & (1U << chan))) {
            continue;
        }
    ...
    for (uint8_t di = 0; di < RP2xxx_NR_PWM_PERIPH_ENABLED; di++) {
